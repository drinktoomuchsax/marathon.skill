...
1. **任务目录**（绝对路径）。两种典型情形：
   - **新建项目型任务**（如"做一个 XXX 系统"）→ 建议一个新建空目录
   - **改动现有仓库型任务**（如"重构 XX 模块"、"给 YY 加测试"）→ 就在仓库根目录运行，**agent 需要改业务代码**
2. **任务标题**（短，用于文件头和日志识别）
3. **时长**（默认 12h，接受 `2h / 4h / 8h / 12h / 24h` 或自由数字）
4. **任务范围**（用户一句话即可，你负责在 task.md 里展开）

若是「改动现有仓库型任务」，额外提示用户在启动前：
- 提交或 stash 现有未提交改动（给 agent 一个干净基线，事后好 diff 回看）
- 创建一个专属分支（如 `marathon/<task-name>`），防污染主分支
- 确认 task.md 的「非范围」里已写明"不要 git push / 不要删除其他模块文件 / 改动限定在 [某某目录/模块]"（由用户根据任务边界自行圈定）
